...
/****************************************************************************
* Adaptado del ejemplo de Blufi de Espressif:
* https://github.com/espressif/esp-idf/tree/v5.1.3/examples/bluetooth
****************************************************************************/
...
#include "blufi_api_wrapper.h"
#include "esp_blufi.h"

#define EXAMPLE_WIFI_CONNECTION_MAXIMUM_RETRY CONFIG_EXAMPLE_MAXIMUM_RETRY
#define EXAMPLE_INVALID_REASON                255
#define EXAMPLE_INVALID_RSSI                  -128

static TaskHandle_t task_to_unblock = NULL;

static void blufi_manager_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param);
...
//        info.sta_ssid_len = gl_sta_ssid_len;
        ...
        BLUFI_INFO("Hemos obtenido IP del AP");
        gl_sta_got_ip = true;
        ...
        if (ble_is_connected == true)
        {
        	...
        	// Hemos logrado la conexion, desinicializamos blufi
        	ret_deinit = esp_blufi_host_deinit();

        	if (ret_deinit == ESP_OK)
        	{
        		BLUFI_INFO("Desinicializacion de Blufi OK");
        	}
        	else
        	{
        		BLUFI_ERROR("Ha ocurrido un error en la desinicializacion de Blufi: %s",
        				esp_err_to_name(ret_deinit));
        	}
        }
        else
        {
        ...
        // Desbloquear caller task incondicionalmente si tenemos conexión al AP
        BLUFI_INFO("Desbloqueamos caller task");
...
static esp_blufi_callbacks_t blufi_manager_callbacks = {
    .event_cb = blufi_manager_event_callback,
...
static void blufi_manager_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
	...
	case ESP_BLUFI_EVENT_RECV_STA_BSSID:
        ...
        BLUFI_INFO("Recv STA BSSID %s\n, wifi config result: %s\n",
        		sta_config.sta.ssid, esp_err_to_name(ret));
        break;
	...
	case ESP_BLUFI_EVENT_RECV_STA_PASSWD:
        ...
        BLUFI_INFO("Recv STA PASSWORD %s\n, wifi config result: %s\n",
        		sta_config.sta.password, esp_err_to_name(ret));
        break;
...
void blufi_manager_start(TaskHandle_t caller_task)
{
    esp_err_t ret;

    assert(caller_task != NULL);
    task_to_unblock = caller_task;

    // Suponemos NVS ya inicializado anteriormente
    ...
    ret = esp_blufi_host_and_cb_init(&blufi_manager_callbacks);
    if (ret) {
...
bool blufi_manager_wifi_is_connected(void)
{
	return gl_sta_got_ip;
}

void blufi_manager_block_until_provisioning_ends()
{
	BLUFI_INFO("Blocking caller\n");

	ulTaskNotifyTake(pdTRUE, portMAX_DELAY );
}

esp_err_t blufi_manager_release_wifi_config()
{
	return esp_wifi_restore();
}

void blufi_manager_start_wifi_only(TaskHandle_t caller_task)
{
	assert(caller_task != NULL);
